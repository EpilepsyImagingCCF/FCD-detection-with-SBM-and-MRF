...
for h in hemis:
    for s in subject_ids:
       #only do lesion mask is present
        if os.path.isfile(os.path.join(subject_dir, s, 'surf_meld/',h+'_lesion.mgh')):
            demo=nb.load(os.path.join(subject_dir, s, 'surf_meld/',h+'_lesion.mgh'))
            lesion=io.import_mgh(os.path.join(subject_dir, s, 'surf_meld/',h+'_lesion.mgh'))
...
#            defragged[~cortex]=0
            io.save_mgh(os.path.join(subject_dir,s,'surf_meld/',h+'_lesion_linked.mgh'),defragged,demo)
